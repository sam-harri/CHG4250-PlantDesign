...
class AcidMixer(UnitInterface):
    ...
    def __size_acid_stream(self):
        ...
        mol_acid_extra_L = (
            AcidMixer.MOLARITY_98WpW_H2SO4 - self.__h2so4_molaity_target
        )  # mol / L
        liters_98acid_needed = (mol_acid_needed / mol_acid_extra_L) 
        mass_98acid_needed = liters_98acid_needed * (AcidMixer.DENSITY_98WpW_H2SO4/1000)
        self.__acid_stream.update_components(
            [H2SO4(mass_98acid_needed * 0.98), Water(mass_98acid_needed * 0.02)]
        )

    def __combine_inlets(self):
        combined_components = Stream.combine_streams([self.__acid_stream, self.__pls_stream])
        ...
        if so4_2m_moles >= uo2_2p_moles:
            updated_components["UO2SO4"] = UO2SO4(flow_rate=uo2_2p_moles, flow_type="molar")
