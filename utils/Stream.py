...
class Stream:
    ...
    def __init__(
        ...
        self.total_molar_flow: float = 0.0  # Added for clarity
    ...
    def _update_totals_and_state_vector(self) -> None:
        ...
        self.total_volume = sum(component.vol_flow for component in self.components if component.has_volume)
    ...
    def get_component_property(self, component_name: str, property_name: str) -> float:
        if component_name not in self.component_indices.keys():
            raise ValueError(f"Component {component_name} not found in stream {self.stream_number}")
        ...
        if property_name == "mass_flow":
            ...
            return component.mol_flow / self.total_molar_flow if self.total_molar_flow > 0 else 0
        elif property_name == "volume_fraction":
            return component.vol_flow / self.total_volume if self.total_volume > 0 and component.has_volume else 0
        else:
    ...
    def __repr__(self) -> str:
        component_details: str = "\n".join([
            ...
            f"Molar flow: {vec[1]:.2f} mol/h, Volume flow: {vec[2]:.2f} m^3/h"
            for idx, (comp, vec) in enumerate(zip(self.components, self.state_vector))
        ])
